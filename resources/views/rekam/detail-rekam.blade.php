                        ...
                        <div class="card-body">
                            <div class="media mb-4 align-items-center">
                                <div class="media-body">
                                    ...
                                    <span
                                        class="fs-14">{{ $pasien->keluhan . ", " . $pasien->kecamatan . ", " . $pasien->kabupaten . ", " . $pasien->kewarganegaraan }}</span>
                                        ...
                                        <div class="col-8 p-0">
                                            @if ($pasien->general_uncent != null)
                                                <a style="width: 120px" class="btn-rounded btn-info btn-xs "
                                                    href="{{ $pasien->getGeneralUncent() }}" target="__BLANK" view>Lihat
                                                    Data</a>
                                            @else
                                                Belum Tersedia
                                            @endif
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive card-table">
                                ...
                                <div class="row">
                                    <div class="col-md-6">
                                        <input type="text" class="form-control gp-search" name="keyword"
                                            value="{{ request("keyword") }}" placeholder="Cari tanggal periksa"
                            ...
                            <tbody>
                                @foreach ($rekams as $key => $row)
                                    <tr>
                                        ...
                                        <td>{{ $row->keluhan }}</td>
                                        <td>
                                            @if ($row->pemeriksaan != null)
                                                {!! $row->pemeriksaan !!}
                                                @if ($row->pemeriksaan_file != null)
                                                    <br>
                                                    <a target="__BLANK" href="{{ $row->getFilePemeriksaan() }}"> <u
                                                            style="color:rgb(28, 85, 231);">Lihat Foto</u> </button>
                                                @endif
                                        </td>
                                @endif
                                <td>
                                ...
                                <td>
                                    @if ($row->tindakan != null)
                                        ...
                                        @if ($row->tindakan_file != null)
                                            <br>
                                            <a target="__BLANK" href="{{ $row->getFileTindakan() }}"> <u
                                                    style="color:rgb(28, 85, 231);">Lihat Foto</u> </button>
                                        @endif
                                </td>
                                @endif
                                <td>
                                            ...
                                            @if ($row->poli != "Poli Gigi")
                                                @if (auth()->user()->role_display() == "Dokter" ||
                                                    ...
                                                    <a href="javascript:void(0)" data-toggle="modal"
                                                        ...
                                                        class="btn-rounded btn-info btn-xs addPemeriksaan"><i
                                                            class="fa fa-pencil"></i> Object</a>
                                                @endif
                                            ...
                                            @else
                                                @if (auth()->user()->role_display() == "Dokter" || auth()->user()->role_display() == "Admin")
                                                    <a href="{{ Route("rekam.gigi.add", $row->id) }}"
                                                        style="width: 120px" class="btn-rounded btn-info btn-xs "><i
                                                            class="fa fa-pencil"></i>
                                                        Rekam</a>

                                                    @if ($row->gigi()->count() > 0)
                                                        <a href="javascript:void(0)" data-toggle="modal"
                                                            data-target="#addResep" data-id="{{ $row->id }}"
                                                            data-tanggal="{{ $row->tgl_rekam }}"
                                    ...
                                    @else
                                        <div class="d-flex">
                                            <a href="{{ Route("obat.pengeluaran", $row->id) }}" style="width: 120px"
                        ...
                        <div class="dataTables_info" id="example_info" role="status" aria-live="polite">Showing
                            {{ $rekams->firstItem() }} to {{ $rekams->perPage() * $rekams->currentPage() }} of
                            {{ $rekams->total() }} entries</div>
